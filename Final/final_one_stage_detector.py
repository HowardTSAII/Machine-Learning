...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        stem_cls.extend([conv, nn.ReLU()])
        stem_box.extend([conv, nn.ReLU()])
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        p_l = ["p3", "p4", "p5"]
        for i in p_l:
            class_logits[i] = self.pred_cls(self.stem_cls(feats_per_fpn_level[i])).flatten(start_dim=2).permute(0, 2, 1)
            boxreg_deltas[i] = self.pred_box(self.stem_box(feats_per_fpn_level[i])).flatten(start_dim=2).permute(0, 2, 1)
            centerness_logits[i] = self.pred_ctr(self.stem_box(feats_per_fpn_level[i])).flatten(start_dim=2).permute(0, 2, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    output_boxes = torch.zeros(deltas.shape[0], 4, device=locations.device)
    output_boxes[:, 0] = locations[:,0] - torch.clamp(deltas[:, 0], min=0) * stride #x1
    output_boxes[:, 1] = locations[:,1] - torch.clamp(deltas[:, 1], min=0) * stride #y1
    output_boxes[:, 2] = locations[:,0] + torch.clamp(deltas[:, 2], min=0) * stride #x2
    output_boxes[:, 3] = locations[:,1] + torch.clamp(deltas[:, 3], min=0) * stride #y2
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = ((torch.min(deltas[:, 0], deltas[:, 2]) * torch.min(deltas[:, 1], deltas[:, 3]))/(torch.max(deltas[:, 0], deltas[:, 2]) * torch.max(deltas[:, 1], deltas[:, 3]))) ** 0.5
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        ######################################################################
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = self.pred_net(self.backbone(images))
        ######################################################################
        ...
        for name,feat in fpn_feats.items():
            shapes[name] = feat.shape
        locations_per_fpn_level = get_fpn_location_coords(shapes, self.backbone.fpn_strides, device=images.device)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for i in range(gt_boxes.shape[0]):
            fpnlevel = fcos_match_locations_to_gt(locations_per_fpn_level, self.backbone.fpn_strides, gt_boxes[i])
            matched_gt_boxes.append(fpnlevel)
        ...
        for i in range(gt_boxes.shape[0]):
            ...
            for key in locations_per_fpn_level.keys():
                matched = fcos_get_deltas_from_locations(locations_per_fpn_level[key], matched_gt_boxes[i][key], self.backbone.fpn_strides[key])
                twmp[key] = matched
            matched_gt_deltas.append(twmp)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # positions to zero.
        ######################################################################
        onehot = F.one_hot(matched_gt_boxes[:, :, 4].long() + 1, num_classes=self.num_classes + 1)[:, :, 1:]
        loss_cls = sigmoid_focal_loss(pred_cls_logits, onehot.float())
        loss_box = 0.25 * F.smooth_l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction="none")
        loss_box[matched_gt_deltas < 0] *= 0.0
        ctr = torch.zeros_like(pred_ctr_logits)
        for i in range(gt_boxes.shape[0]):
            ctr[i,:,:] = fcos_make_centerness_targets(matched_gt_deltas[i,:,:]).reshape(-1,1)
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits.reshape(-1), ctr.reshape(-1), reduction="none")
        loss_ctr[ctr.reshape(-1) < 0] *= 0
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            score= torch.max(level_pred_scores, dim=1).values
            level_pred_classes = torch.argmax(level_pred_scores, dim=1)
            level_pred_scores = score
            ...
            level_pred_boxes[:, 0] = torch.clamp(level_pred_boxes[:, 0], min=0, max=images.shape[3])
            level_pred_boxes[:, 1] = torch.clamp(level_pred_boxes[:, 1], min=0, max=images.shape[2])
            level_pred_boxes[:, 2] = torch.clamp(level_pred_boxes[:, 2], min=0, max=images.shape[3])
            level_pred_boxes[:, 3] = torch.clamp(level_pred_boxes[:, 3], min=0, max=images.shape[2])            
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
