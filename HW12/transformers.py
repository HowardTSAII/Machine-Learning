...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        torch.nn.init.xavier_uniform_(self.k.weight.data)
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        y, self.weight_softmax = scaled_dot_product_no_loop_batch(self.q(query), self.k(key), self.v(value), mask)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.selfattn = nn.ModuleList([SelfAttention(dim_in, dim_out, dim_out) for _ in range(num_heads)])
        #self.selfattn1 = SelfAttention(dim_in, dim_out, dim_out)
        #self.selfattn2 = SelfAttention(dim_in, dim_out, dim_out)
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        y = self.linear(torch.cat([h(query, key, value, mask) for h in self.selfattn], dim=-1))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        x1 = self.dropout(self.norm1(x + self.attn(x, x, x)))
