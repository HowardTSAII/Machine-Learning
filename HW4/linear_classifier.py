...
def svm_loss_vectorized(W, X, y, reg):
  ...
  margin = torch.maximum(torch.zeros((num_classes)).to(device),scores - correct_class_scores + 1)
  ...
  margin[torch.arange(num_train), y] -= valid_margin_count
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  loss = cls.train(X_train, y_train, learning_rate=lr,
                    reg=reg, num_iters=100)
  ...
  return cls, train_acc.cpu().numpy(), val_acc.cpu().numpy()
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    for j in range(num_classes):
        denom += torch.exp(scores[i, j])
    # print(denom)
    l_i = -1 * scores[i, y[i]] + torch.log(denom)
    loss += l_i
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores = X.mm(W)
  # make largest value here zero, just for numeric stability
  # scores -=torch.max(torch.max(scores))
  maximum_score = torch.max(scores, 1)[0].reshape(-1, 1)
  scores -= maximum_score

  expscores = torch.exp(scores)

  denoms = torch.sum(expscores, 1)
  ...
  device = torch.device("cuda")
  correct_class = torch.zeros((num_train, num_classes), dtype=X.dtype).to(device)
